...
instance MonadIO Enclave where
  liftIO io = Enclave io


data Secure a = SecureDummy

{-# NOINLINE inEnclaveConstant #-}
{-# RULES "HASTEERULES inEnclaveConstant/Enclave" forall x . inEnclaveConstant x = return (return x) #-}
inEnclaveConstant :: a -> App (Enclave a)
inEnclaveConstant _ = error "inEnclaveConstant error"
-- inEnclaveConstant = return . return

{-# NOINLINE liftNewRef #-}
{-# RULES "HASTEERULES liftNewRef/Enclave" forall x . liftNewRef x = App (do r <- liftIO $ newIORef x; return (return r)) #-}
liftNewRef :: a -> App (Enclave (Ref a))
liftNewRef a = App $ do
  r <- liftIO $ newIORef a
  return (return r)
...
{-# NOINLINE newRef #-}
{-# RULES "HASTEERULES newRef/Enclave" forall v . newRef v = Enclave (newIORef v) #-}
newRef :: a -> Enclave (Ref a)
newRef x = Enclave $ newIORef x

{-# NOINLINE readRef #-}
{-# RULES "HASTEERULES readRef/Enclave" forall ref . readRef ref = Enclave (readIORef ref) #-}
readRef :: Ref a -> Enclave a
readRef ref = Enclave $ readIORef ref

{-# NOINLINE writeRef #-}
{-# RULES "HASTEERULES writeRef/Enclave" forall ref v . writeRef ref v = Enclave (writeIORef ref v) #-}
writeRef :: Ref a -> a -> Enclave ()
writeRef ref v = Enclave $ writeIORef ref v

{-# NOINLINE inEnclave #-}
{-# RULES "HASTEERULES inEnclave/Enclave" forall f . inEnclave f = App $ do
                                                                            (next_id, remotes) <- get
                                                                            put (next_id + 1, (next_id, \bs -> let Enclave n = mkSecure f bs in n) : remotes)
                                                                            return SecureDummy #-}
inEnclave :: (Securable a) => a -> App (Secure a)
...
{-# NOINLINE (<@>) #-}
{-# RULES "HASTEERULES HASTEERULES app/Enclave" forall closure arg . (<@>) closure arg = error "enclave can not apply client-side closures" #-}
(<@>) :: Binary a => Secure (a -> b) -> a -> Secure b
(<@>) = error "Access to client not allowed"
...
{-# NOINLINE runClient #-}
{-# RULES "HASTEERULES runClient/Enclave" forall client . runClient client = return Done #-}
runClient :: Client a -> App Done
runClient _ = error "runClient error"
-- runClient _ = return Done

{-# NOINLINE tryEnclave #-}
{-# RULES "HASTEERULES tryEnclave/Enclave" forall closure . tryEnclave closure = ClientDummy #-}
tryEnclave :: (Binary a) => Secure (Enclave a) -> Client (Maybe a)
tryEnclave _ = error "tryEnclave error"
-- tryEnclave _ = ClientDummy

{-# NOINLINE gateway #-}
{-# RULES "HASTEERULES gateway/Enclave" forall closure . gateway closure = ClientDummy #-}
gateway :: Binary a => Secure (Enclave a) -> Client a
gateway _ = error "gateway error"
-- gateway _ = ClientDummy

{-# NOINLINE unsafeOnEnclave #-}
{-# RULES "HASTEERULES unsafeOnEnclave/Enclave" forall closure . unsafeOnEnclave closure = ClientDummy #-}
unsafeOnEnclave :: Binary a => Secure (Enclave a) -> Client a
unsafeOnEnclave _ = error "unsafeOnEnclave error"
-- unsafeOnEnclave _ = ClientDummy

{-# NOINLINE runApp #-}
{-# RULES "HASTEERULES runApp/Enclave" forall app . runApp app = do (_, (_,vTable)) <- runStateT (unApp app) initAppState
                                                                    _ <- serve (Host localhost) connectPort $
...
onEvent mapping incoming socket = do
  let (identifier, args) = decode incoming :: (CallID, [ByteString])
      Just f = lookup identifier mapping
  putStrLn $ "received request for id: " <> show identifier
  result <- encode <$> f args
  let res = handleVoidTy result -- the () type cannot be sent over wire
  sendLazy socket (B.append (msgSize res) res) -- See NOTE 1
  where
